...
static createFilmCard(film) {
  ...
  const favoriteBadge = film.favorite
    ? `<span class="badge badge-danger">♥ Favori</span>`
    : `<span class="badge badge-secondary">Normal</span>`;

  const statusButtonText =
    film.status === "watched" ? "Watchlist'e Al" : "İzlendi Yap";

  const favoriteButtonText =
    film.favorite ? "Favoriden Çıkar" : "Favori Yap";

  const stars = Array.from({ length: 5 }, (_, i) => {
    return `<span class="star ${i < film.rating ? "filled" : ""}" data-id="${film.id}" data-rating="${i + 1}">★</span>`;
  }).join("");
  ...
  return `
    ...
    <div class="movie-card-body">
      ...
      <p class="movie-year">${film.year || "Bilinmiyor"}</p>
  ...
  static updateFilmCount(count = null) {
    ...
    if (filmCountElement) {
      const total =
  count !== null ? count : document.querySelectorAll("#films .movie-card").length;
